...
def test_transcription_worker_passes_mode():
    ...
    assert worker_default.mode == "clean"
    worker_default.run()
    mock_service.transcribe.assert_called_once_with(wav_bytes, "clean")
    
    # Test worker with custom mode
    mock_service.reset_mock()
    worker_clean = TranscriptionWorker(mock_service, wav_bytes, mode="translate")
    assert worker_clean.mode == "translate"
    worker_clean.run()
    mock_service.transcribe.assert_called_once_with(wav_bytes, "translate")
